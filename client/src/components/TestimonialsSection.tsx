...
/* =========================
   ...
   Inserted as a <style> block when used.
   ========================= */
const marqueeCss = `
:root {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="overflow-hidden w-full">
          <div
            ...
            style={{
              width: `${(videos.length * 100) / itemsPerView}%`,
              transform: `translateX(-${(startIndex * 100) / videos.length}%)`,
            }}
          >
            {videos.map((v, idx) => {
              const visibleWidth = `${itemWidthPercent}%`;
              const isCenter = idx === centerIndex;
              // scale center item slightly
              return (
                <div
                  key={v.id}
                  className="px-2"
                  style={{
                    flex: `0 0 ${visibleWidth}`,
                    maxWidth: visibleWidth,
                    alignSelf: isCenter ? "center" : "stretch",
                  }}
                >
                  <article
                    className={`bg-white rounded-lg overflow-hidden border border-gray-100 flex flex-col h-[540px] max-h-[540px]`}
                  >
                    <div className={`relative overflow-hidden rounded-t-lg h-72 md:h-80 lg:h-96 bg-gray-100`}>
                      ...
                      <div
                        aria-hidden
                        className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-green-700/95 flex items-center justify-center shadow"
                    ...
                    <div className="p-4 flex-1 overflow-auto">
                      ...
                      <div className="mt-4">
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-3 py-2 bg-green-700 text-white rounded-md text-sm shadow"
                        >
...
/* =========================
   ...
   - combines client & candidate carousels + written marquee
   ========================= */
export default function TestimonialsSection() {
  // handler for playing a video excerpt: open modal or navigate - here we open a small overlay
    ...
    <section className="px-6 py-10">
      ...
      <div className="mb-14">
        ...
        <VideoCarousel
          ...
          title={undefined}
          onPlay={(src) => setPlayingSrc(src)}
        />
      </div>

      {/* CANDIDATE VIDEO TESTIMONIALS */}
      <div className="mb-14">
      ...
      {/* Candidate written testimonials */}
      <div>
